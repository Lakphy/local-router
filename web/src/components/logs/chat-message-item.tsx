...
function CopyablePre({
  ...
  return (
    <div className="relative">
      ...
      <pre
        className={cn('overflow-auto rounded-md border bg-background p-2 pr-10 text-xs', className)}
      >
        {text}
      </pre>
    </div>
...
function renderBlock(block: NormalizedContentBlock, index: number): React.ReactNode {
  ...
  return (
    <BlockContainer key={index} title={block.label ?? 'unknown'} defaultOpen={false}>
      <CopyablePre text={prettyJson(block.raw)} copyLabel="unknown block" className="max-h-52" />
    </BlockContainer>
  );
}
  ...
  const blocksContent = (
    <div className="space-y-2">
      {message.blocks.length > 0
        ? message.blocks.map((block, blockIndex) => renderBlock(block, blockIndex))
        : null}
    </div>
  );
