...
export function RoutesEditor() {
  ...
  function defaultTarget() {
    const provider = Object.keys(currentDraft.providers)[0] ?? '';
    const model = provider
      ? (Object.keys(currentDraft.providers[provider]?.models ?? {})[0] ?? '')
      : '';
  ...
  function addRouteType() {
    if (!addType) return;
    updateDraft((cfg) => {
      cfg.routes[addType] = { '*': defaultTarget() };
      return cfg;
    });
    setAddType('');
  }
