/**
 * OpenAI Responses → Anthropic Messages 适配器
 *
 * 客户端使用 OpenAI Responses 协议 (/v1/responses)，
 * 上游 provider 使用 Anthropic Messages 协议 (/v1/messages)。
 *
 * 配置示例（零参数）：
 *   { "package": "./packages/plugin-adapter-responses-to-anthropic" }
 */
...
const SOURCE = 'openai-responses' as const;
const TARGET = 'anthropic-messages' as const;

const definition: PluginDefinition = {
  name: 'adapter-responses-to-anthropic',
  version: '0.1.0',
