...
import { emitResult, runCommand, startStream } from '../output';
    ...
    runCommand({
      ...
      fn: async (ctx) => {
        const parsed = parseArgs({
          ...
          options: {
            config: { type: 'string' },
            'skip-providers': { type: 'boolean', default: false },
          },
          allowPositionals: true,
          strict: false,
        });
        const checks: DoctorCheck[] = [];
        const { path, config, error: configErr } = tryRead(parsed.values.config);
        if (configErr || !config) {
          checks.push({
            name: '配置文件',
            ok: false,
        ...
        if (config && !parsed.values['skip-providers']) {
        ...
        emitResult(ctx, {
          command: 'doctor',
          data: { ok: allOk, passed, total, checks },
          md: {
            heading: `doctor · ${passed}/${total} 通过`,
  ...
  handler: async (args, flags) =>
    runCommand({
      ...
      fn: (ctx) => {
        ...
        if (!entry || !reqModel) {
          throw new CliError('USAGE_ERROR', '用法: explain route --entry <e> --model <m>');
        }
  ...
  handler: async (args, flags) =>
    runCommand({
      ...
      fn: async (ctx) => {
        ...
        const prompt = parsed.values.prompt ?? 'ping';
        if (!entry || !model) {
          throw new CliError('USAGE_ERROR', '用法: try --entry <e> --model <m> --prompt <text>');
        }
        const timeoutSec = parsed.values.timeout ? Number.parseInt(parsed.values.timeout, 10) : 30;
